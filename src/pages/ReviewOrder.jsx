...
export default function ReviewOrder() {
  ...
  const PAY_OPTIONS = [
    { id: 'razorpay', icon: '💳', label: 'Pay Online', sub: 'UPI · Card · Wallet via Razorpay' },
    { id: 'cod',      icon: '💵', label: 'Cash on Delivery', sub: 'Pay when your basket arrives' },
  ]
  ...
  return (
    <div className="page-shell">
      ...
      <div className="page-shell-scroll" style={{ padding:'14px 18px 130px', display:'flex', flexDirection:'column', gap:12 }}>
        ...
        <div>
          <div style={{ fontWeight:600, fontSize:15, marginBottom:10 }}>Payment Method</div>
          <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
            {PAY_OPTIONS.map(opt => {
              ...
              return (
                <button key={opt.id} onClick={() => setPayMethod(opt.id)} style={{
                  ...
                  <div style={{ flex:1 }}>
                    <div style={{ fontWeight:700, fontSize:14, color: sel ? 'var(--green)' : 'var(--text)' }}>{opt.label}</div>
                    <div style={{ fontSize:11, color:'var(--text-light)', marginTop:2 }}>{opt.sub}</div>
                  </div>
                  <div style={{
        ...
        <div className="card">
          ...
          {payMethod === 'cod' && (
            <div style={{ fontSize:12, color:'#B7791F', marginTop:8, background:'#FFF8E1', padding:'6px 10px', borderRadius:8 }}>
              💵 Pay ₹{total} in cash when your basket is delivered
            </div>
          )}
          {payMethod === 'razorpay' && (
            <div style={{ fontSize:12, color:'var(--green)', marginTop:8, background:'var(--green-pale)', padding:'6px 10px', borderRadius:8 }}>
      ...
      <div className="sticky-footer">
        ...
        <button className="btn btn-primary" onClick={() => nav('/schedule', { state: { ...state, totalAmount: total, baskets, basket: baskets[0], paymentMethod: payMethod } })}>
          {payMethod === 'razorpay' ? 'Proceed to Payment →' : 'Schedule Delivery →'}
        </button>
      </div>
    </div>
  )
