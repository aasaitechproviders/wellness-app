...
/* ═══════════════════════════════════════════════════════════════
   IncomingCallOverlay — shows when nutritionist is calling
   ═══════════════════════════════════════════════════════════════ */
function IncomingCallOverlay({ appointment, onEnd }) {
  const [status, setStatus]       = useState('ringing') // ringing → in_call → ended
  ...
  const acceptCall = async () => {
    try {
      ...
      pc.onicecandidate = (e) => {
        if (e.candidate) {
          api.sendSignal({ appointmentId: aptId, type: 'ice-candidate', data: JSON.stringify(e.candidate), from: 'customer' }).catch(() => {})
        }
      }
      ...
      await api.updateCallStatus(aptId, 'accepted')

      // Poll for admin's SDP offer and ICE candidates
      pollRef.current = setInterval(async () => {
        try {
          const d = await api.getSignals(aptId)
          for (const sig of (d.signals || [])) {
            ...
            if (sig.type === 'offer') {
              ...
              await api.sendSignal({ appointmentId: aptId, type: 'answer', data: JSON.stringify(answer), from: 'customer' })
            } else if (sig.type === 'ice-candidate') {
              try { await pc.addIceCandidate(JSON.parse(sig.data)) } catch {}
            } else if (sig.type === 'end-call') {
              endCall()
            }
          }
        } catch {}
    ...
    <div style={{ position:'fixed',inset:0,zIndex:9999,background:'linear-gradient(135deg,#1A3D20 0%,#2D6A35 50%,#1A3D20 100%)',
      ...
      <div style={{ color:'#fff',textAlign:'center' }}>
        <div style={{ fontSize:20,fontWeight:700,marginBottom:4 }}>
          {appointment.assignedTo || 'Nutritionist'}
        </div>
...
export default function Appointment() {
  ...
  const [incomingCall, setIncomingCall] = useState(null) // appointment object when ringing
  ...
  // From navigation state (health conditions that triggered this)
  ...
  // Poll for incoming calls on assigned appointments
  useEffect(() => {
  ...
  const needsBooking = showBooking || (!loading && triggerConditions.length > 0 && !appointments.some(a => ['requested','assigned'].includes(a.status) && a.memberId === triggerMemberId))
      ...
      <div className="page-shell-scroll with-nav" style={{ padding:'16px 18px' }}>
          ...
          <div style={{ marginBottom:16 }}>
            ...
            <div style={{ display:'flex',flexWrap:'wrap',gap:6 }}>
              {triggerConditions.map(c => (
                <span key={c} style={{ padding:'5px 12px',borderRadius:20,background:'#FFF3E0',border:'1.5px solid #E65100',
        ...
        {needsBooking && (
          <div style={{ background:'#fff',borderRadius:14,border:'1.5px solid var(--border)',padding:'16px',marginBottom:16 }}>
            ...
            {/* Date picker */}
            <div style={{ fontSize:11,fontWeight:700,color:'var(--text-mid)',textTransform:'uppercase',letterSpacing:0.5,marginBottom:8 }}>
              Select Date
            </div>
                ...
                return (
                  <button key={s} onClick={() => setSelSlot(s)}
                    style={{ padding:'10px 6px',borderRadius:10,border:`1.5px solid ${sel?'var(--green)':'var(--border)'}`,
                      background:sel?'var(--green-pale)':'#fff',color:sel?'var(--green)':'var(--text-mid)',
            ...
            <button className="btn btn-primary" onClick={requestAppointment} disabled={submitting}
              style={{ width:'100%' }}>
        ...
        {loading ? (
          ...
          appointments.map(apt => {
            ...
            return (
              <div key={apt._id} style={{ background:'#fff',borderRadius:14,border:'1.5px solid var(--border)',
                ...
                <div style={{ display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:8 }}>
                  ...
                  <span style={{ padding:'4px 10px',borderRadius:20,background:ui.bg,color:ui.color,
                ...
                {(apt.healthConditions||[]).length > 0 && (
                  <div style={{ display:'flex',flexWrap:'wrap',gap:4,marginBottom:8 }}>
                    {apt.healthConditions.map(c => (
                      <span key={c} style={{ padding:'2px 8px',borderRadius:12,background:'#FFF3E0',color:'#E65100',fontSize:10,fontWeight:600 }}>
                        {c}
                      </span>
                    ))}
                  </div>
                ...
                {apt.status === 'completed' && apt.totalCalories && (
                  <div style={{ background:'var(--green-pale)',borderRadius:10,padding:'10px 14px',marginBottom:8 }}>
                    ...
                    {apt.nutritionistNotes && (
                      <div style={{ fontSize:12,color:'var(--text-mid)',marginTop:6,fontStyle:'italic' }}>
                        "{apt.nutritionistNotes}"
                      </div>
                    )}
                  </div>
